...
const op = setInterval(incNum, 20);
        ...
        function updateProgress() {
            ...
            const fullWidth = 500; // Width defined in CSS
            const progress = Math.min(Math.floor((lineWidth / fullWidth) * 100), 100);
            text.innerHTML = progress + '%';

            // Stop updating and remove elements when progress reaches 100%
            if (progress >= 100) {
                clearInterval(updateInterval);
                setTimeout(() => {
                    document.querySelector('.main').style.display = 'none'; // Hide the progress elements
                }, 100); // Delay to allow final update to be visible
            }
        }

        // Update progress every 50ms
        const updateInterval = setInterval(updateProgress, 20);